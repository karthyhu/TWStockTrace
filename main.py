    ...
    try:
        ...
        if top_gainers:
            ...
            for i, stock in enumerate(top_gainers, 1):
                ...
                if range_val > 0:
                    gainers_text += f"{i}. **{stock['Code']}** {stock['Name']}: +{range_val:.2f}%\n"
                else:
                    gainers_text += f"{i}. **{stock['Code']}** {stock['Name']}: {range_val:.2f}%\n"
        ...
        if top_losers:
            losers_text = ""
            for i, stock in enumerate(reversed(top_losers), 1):  # 反轉順序，最跌的排第一
                range_val = stock['Range']
                losers_text += f"{i}. **{stock['Code']}** {stock['Name']}: {range_val:.2f}%\n"
